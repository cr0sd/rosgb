...
void decexecCB(St*,uint8_t*,uint8_t*,uint8_t*);
...
// write_ram Allow access to read/write to ram
...
void write_ram(uint16_t addr,uint8_t val,St *st,uint8_t *ram)
{
	...
	{
		updatejoypad(ram);//this is distinct from interrupts (?)
		// st->sp-=2;
		// // write_ram(st->sp,(uint16_t)st->pc&0x00ff,st,ram);
		// // write_ram(st->sp+1,(uint16_t)st->pc>>8,st,ram);
		// ram[(uint16_t)st->pc&0x00ff]=st->sp;
		// ram[(uint16_t)st->pc>>8]=st->sp+1;
...
void decexec(St *st,uint8_t *rom,uint8_t *op,uint8_t *ram)
{
	...
	case 0x02://ld (bc),a
		...
		write_ram(st->b<<8|st->c,st->a,st,ram);
	...
	case 0x12://ld (de),a
		...
		write_ram(st->d<<8|st->e,st->a,st,ram);
		printf("ld (de),a");
		break;
	case 0x13://inc de
	...
	case 0x34://inc (hl)
		tmp=st->h|st->l<<8;
		// ram[tmp]++;
		write_ram(tmp,ram[tmp]+1,st,ram);
	...
	case 0x36://ld (hl),m8
		...
		write_ram(st->h<<8|st->l,(uint8_t)tmp,st,ram);
	...
	case 0x70: //ld (hl),b
		tmp=st->h<<8|st->l;
		// ram[tmp]=st->b;
		write_ram(tmp,st->b,st,ram);
		printf("ld (hl),b ;ld (%.4xh),%.2xh",tmp,st->b);
		break;
	case 0x71: //ld (hl),c
		tmp=st->h<<8|st->l;
		// ram[tmp]=st->c;
		write_ram(tmp,st->c,st,ram);
		printf("ld (hl),c");
		break;
	case 0x72: //ld (hl),d
		tmp=st->h<<8|st->l;
		// ram[tmp]=st->d;
		write_ram(tmp,st->d,st,ram);
		printf("ld (hl),d");
		break;
	case 0x73: //ld (hl),e
		tmp=st->h<<8|st->l;
		// ram[tmp]=st->e;
		write_ram(tmp,st->e,st,ram);
		printf("ld (hl),e");
		break;
	case 0x74: //ld (hl),h
		tmp=st->h<<8|st->l;
		// ram[tmp]=st->h;
		write_ram(tmp,st->h,st,ram);
		printf("ld (hl),h");
		break;
	case 0x75: //ld (hl),l
		tmp=st->h<<8|st->l;
		// ram[tmp]=st->l;
		write_ram(tmp,st->l,st,ram);
		printf("ld (hl),l");
		break;
	case 0x76: //halt
		//if(st->ime)
		st->pc--;//stay put
		printf("halt%s",ram[0xffff]?"":"");
		break;
	case 0x77: //ld (hl),a
		tmp=st->h<<8|st->l;
		// ram[tmp]=st->a;
	...
	case 0x95://sub l
		...
		st->f|=F_N;//N
		printf("sub l ;CF:%x a:%.2x",(st->f&F_C),st->a);
		break;
	case 0x96://sub (hl)
		tmp=(uint16_t)st->h<<8|st->l;
	...
	case 0xc4://call nz,m16
		...
		if(!(st->f&F_Z))
		{
			...
			// ram[st->sp+1]=st->pc>>8;
			write_ram(st->sp,(uint8_t)st->pc,st,ram);
			write_ram(st->sp+1,st->pc>>8,st,ram);
			st->pc=tmp;
		}
	...
	case 0xc5://push bc
		...
		// ram[st->sp+1]=st->b;
		write_ram(st->sp,st->c,st,ram);
		write_ram(st->sp+1,st->b,st,ram);
	...
	case 0xc7: //rst 00h (call 0000h + 00h)
		...
		// ram[st->sp+1]=st->pc>>8;
		write_ram(st->sp,st->pc&0x00ff,st,ram);
		write_ram(st->sp+1,st->pc>>8,st,ram);
		st->pc=0x0000;
	...
	case 0xd5://push de
		...
		// ram[st->sp+1]=st->d;
		write_ram(st->sp,st->e,st,ram);
		write_ram(st->sp+1,st->d,st,ram);
	...
	case 0xe0://ldh (m8),a ---> ld (ff00 + m8),a
		fetch8(st,rom,&tmp,ram);
		// ram[0xff00|tmp]=st->a;
		write_ram(0xff00|tmp,st->a,st,ram);
	...
	case 0xe2://ld (c),a
		...
		write_ram(0xff00|(uint16_t)st->c,st->a,st,ram);
		printf("ld (c),a");
		break;
	case 0xe5://push hl
		st->sp-=2;
		// ram[st->sp]=st->h;
		// ram[st->sp+1]=st->l;
		write_ram(st->sp,st->h,st,ram);
		write_ram(st->sp+1,st->l,st,ram);
	...
	case 0xf5://push af
		...
		// ram[st->sp]=st->a;
		// ram[st->sp+1]=st->f;
		write_ram(st->sp,st->a,st,ram);
		write_ram(st->sp+1,st->f,st,ram);
